...
import { Config, readConfig, webpackConfiguration } from "./config.js"
...
async function build (argv: ArgumentsCamelCase, builtHandler: () => void = () => {}, webpackHandler: (compiler: webpack.Compiler) => void = () => {}): Promise<void> {
    let config: Config
    try {
        config = await readConfig(pathToFileURL(argv.config as string | null ?? "./cheat-menu-config.js").href)
    } catch (e) {
    ...
    const buildingText = chalkAnimation.rainbow(
        "Building..."
    )
    ...
    compiler.run(async (err, stats) => {
        ...
        if (argv["generate-bookmarklet"] as boolean) {
...
\ncreate()\n`)
    ...
    .command("dev", "Watch for changes, build, then hot reload cheat menu.", (yargs) => {
        ...
        let currentBundle: string | Buffer
